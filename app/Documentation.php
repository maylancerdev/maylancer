...
class Documentation
{
    ...
    {
        $basePath = url('/docs/' . ltrim($pathPrefix, '/'));


       $rendered =  Markdown::convert($text)->getContent();

        $rendered =  $this->replaceMarkdownPaths($rendered, $basePath);
    ...
    public function replaceImagePath($rendered, $doc, $version): array|string|null
    {


        $searchImagePath = '/images';
        $imagePath = asset($this->getAssetPath($doc, $version) . '/images');

        $pattern = '/(<img\s+src=["\'])(.*?)' . preg_quote($searchImagePath, '/') . '(.*?)(".*?>)/i';
        $rendered = preg_replace_callback($pattern, function ($matches) use ($imagePath) {
            $oldImageLink = $matches[2];
            $newImageLink = $imagePath . $oldImageLink;
            $replacement = $matches[1] . $newImageLink . $matches[3] . $matches[4];
            return $replacement;
        }, $rendered);
    ...
    protected function getAssetPath($doc, $version): string
    {
        // Customize this logic to retrieve the asset path based on the $doc and $version parameters
        // For example, you can construct the path dynamically using configuration values or other means
        $basePath = config('docs.asset_path');
        return $basePath . "/{$doc}/{$version}";
    }



   public function replaceMarkdownPaths($rendered, $basePath)
    {
